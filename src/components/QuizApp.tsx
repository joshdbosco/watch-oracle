  ...
  async function submitForm() {
    ...
    try {
      await fetch('/api/submit', {
        ...
        body: JSON.stringify({ name, email: email.trim(), extra, tags }),
      })
    } catch {
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 36px 140px' }}>
        ...
        {screen === 'intro' && (
          <div style={{ paddingTop: 130, textAlign: 'center', animation: 'rise 1s cubic-bezier(.16,1,.3,1) both' }}>
            ...
            <h1 style={{ fontSize: 'clamp(42px,7.5vw,68px)', fontWeight: 400, lineHeight: 1.05, letterSpacing: '-.02em', marginBottom: 32 }}>
              You have no idea<br />what to buy <em style={{ fontStyle: 'italic', color: 'var(--gold-lt)' }}>them.</em>
            </h1>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 80 }}>
              {QUESTIONS.map((_, i) => (
                <div key={i} style={{
                  ...
                  background: i < question ? 'var(--gold)' : i === question ? 'var(--gold-lt)' : 'var(--border)',
        ...
        {screen === 'thinking' && (
          <div style={{ paddingTop: 150, minHeight: '50vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
            ...
            <div style={{ height: 80, width: 380, maxWidth: '100%', position: 'relative', overflow: 'hidden' }}>
              <p key={phrase} style={{
                fontSize: 20, fontStyle: 'italic', color: 'var(--sub)', lineHeight: 1.5,
        ...
        {screen === 'fork' && (
          <div style={{ paddingTop: 108, animation: 'rise .42s cubic-bezier(.16,1,.3,1) both' }}>
            ...
            <h2 style={{ fontSize: 'clamp(30px,5.5vw,48px)', fontWeight: 400, lineHeight: 1.12, marginBottom: 14 }}>
              We&apos;ve <em style={{ fontStyle: 'italic', color: 'var(--gold-lt)' }}>worked it out.</em>
            </h2>
        ...
        {screen === 'email' && (
          <div style={{ paddingTop: 96, animation: 'rise .42s cubic-bezier(.16,1,.3,1) both' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px,5vw,44px)', fontWeight: 400, lineHeight: 1.15, marginBottom: 12 }}>
              Tell us about <em style={{ fontStyle: 'italic', color: 'var(--gold-lt)' }}>them.</em>
            </h2>
            ...
            <Field label="Your email *">
              ...
              {emailErr && <p style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#b04030', marginTop: 7 }}>Please enter a valid email address.</p>}
            </Field>
            ...
            <div style={{ margin: '36px 0', padding: '24px 28px', borderLeft: '2px solid rgba(184,151,106,.35)', background: 'rgba(184,151,106,.04)' }}>
              <p style={{ fontSize: 17, fontStyle: 'italic', color: 'var(--sub)', lineHeight: 1.85 }}>
                <strong style={{ fontStyle: 'normal', color: 'var(--text)' }}>A real person who loves watches</strong> will read this and reply personally within 24 hours. Not a template. Not a bot.
              </p>
            </div>
        ...
        {screen === 'sent' && (
          <div style={{ paddingTop: 150, textAlign: 'center', animation: 'rise .55s cubic-bezier(.16,1,.3,1) both' }}>
            <div style={{ width: 60, height: 60, border: '1px solid var(--gold)', borderRadius: '50%', margin: '0 auto 44px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, animation: 'pop .55s cubic-bezier(.34,1.56,.64,1) both .15s' }}>✓</div>
        ...
        {screen === 'result' && persona && (
          <div style={{ paddingTop: 88 }}>
            ...
            <p style={{ fontSize: 22, fontStyle: 'italic', color: 'var(--sub)', lineHeight: 1.7, marginBottom: 28 }}>
              {persona.descriptor}
            </p>
            <p style={{ fontSize: 18, fontStyle: 'italic', color: 'var(--muted)', lineHeight: 1.9, marginBottom: 56, paddingLeft: 22, borderLeft: '2px solid rgba(184,151,106,.28)' }}>
              {persona.insight}
            </p>
            ...
            <div style={{ marginTop: 44, padding: 32, border: '1px solid var(--border)', background: 'var(--surface)', textAlign: 'center' }}>
              <p style={{ fontSize: 18, fontStyle: 'italic', color: 'var(--muted)', marginBottom: 24, lineHeight: 1.7 }}>
                Want a second opinion from <strong style={{ color: 'var(--sub)', fontStyle: 'normal' }}>a real watch enthusiast</strong>?<br />
                We&apos;ll reply personally within 24 hours.
              </p>
              <OracleButton onClick={() => go('email')}>Get a personal recommendation →</OracleButton>
            </div>

            <div style={{ marginTop: 60, paddingTop: 36, borderTop: '1px solid var(--border)', textAlign: 'center' }}>
...
function Clock() {
  return (
    <div style={{ width: 52, height: 52, border: '1px solid rgba(184,151,106,.38)', borderRadius: '50%', margin: '0 auto 52px', position: 'relative' }}>
      <div style={{ position: 'absolute', left: '50%', bottom: '50%', transformOrigin: '50% 100%', background: 'var(--gold)', borderRadius: 1, width: 1.5, height: 13, marginLeft: -0.75, animation: 'rot 12s linear infinite' }} />
      <div style={{ position: 'absolute', left: '50%', bottom: '50%', transformOrigin: '50% 100%', background: 'var(--gold)', borderRadius: 1, width: 1, height: 18, marginLeft: -0.5, animation: 'rot 3s linear infinite', opacity: 0.5 }} />
      <div style={{ position: 'absolute', width: 3, height: 3, background: 'var(--gold)', borderRadius: '50%', left: '50%', top: '50%', transform: 'translate(-50%,-50%)' }} />
    </div>
  )
}
...
function ThinkingRing() {
  return (
    <div style={{ width: 70, height: 70, border: '1px solid var(--border)', borderTopColor: 'var(--gold)', borderRadius: '50%', animation: 'spin 2s linear infinite', marginBottom: 56, position: 'relative', flexShrink: 0 }}>
      <div style={{ position: 'absolute', inset: 7, border: '1px solid transparent', borderRightColor: 'rgba(184,151,106,.22)', borderRadius: '50%', animation: 'spinr 3.2s linear infinite' }} />
    </div>
  )
}
  ...
  return (
    <button
      ...
      style={{
        ...
        color: hovered ? 'var(--deep)' : 'var(--gold)',
  ...
  return (
    <button
      ...
      style={{
        ...
        border: '1px solid var(--border)',
        color: hovered ? 'var(--sub)' : 'var(--muted)',
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${sel || hovered ? 'rgba(184,151,106,.4)' : 'var(--border)'}`,
        background: sel || hovered ? 'var(--hover)' : 'var(--surface)',
        color: 'var(--text)',
...
function ForkCard({ icon, title, desc, onClick }: { icon: string; title: string; desc: string; onClick: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        padding: '32px 28px',
        border: `1px solid ${hovered ? 'rgba(184,151,106,.38)' : 'var(--border)'}`,
        background: hovered ? 'var(--hover)' : 'var(--surface)',
        cursor: 'pointer', position: 'relative', overflow: 'hidden',
        transition: 'border-color .25s, background .25s',
      }}
    >
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 2, background: 'var(--gold)', transform: hovered ? 'scaleY(1)' : 'scaleY(0)', transformOrigin: 'bottom', transition: 'transform .3s cubic-bezier(.16,1,.3,1)' }} />
      ...
      <span style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: 11, color: 'var(--muted)', lineHeight: 1.65, letterSpacing: '.02em' }}>{desc}</span>
    </div>
  )
}
...
function Divider({ label }: { label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 28 }}>
      ...
      <span style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: 9, letterSpacing: '.3em', color: 'var(--muted)', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
    </div>
  )
}
...
function WatchCard({ watch, rank }: { watch: Watch; rank: number }) {
  ...
  return (
    <div
      ...
      style={{
        padding: 30,
        border: `1px solid ${isTop ? 'rgba(184,151,106,.45)' : hovered ? 'rgba(184,151,106,.28)' : 'var(--border)'}`,
        background: hovered ? 'var(--hover)' : 'var(--surface)',
    ...
    >
      ...
      {isTop && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: 'linear-gradient(90deg, transparent, var(--gold), transparent)' }} />}

      <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: 9, letterSpacing: '.22em', color: 'var(--gold)', textTransform: 'uppercase', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
        {labels[rank]}
        <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
      </div>
      <div style={{ fontSize: 28, fontWeight: 500, marginBottom: 4, lineHeight: 1.2 }}>{watch.name}</div>
      <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: 10, letterSpacing: '.22em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 18 }}>{watch.brand}</div>
      <div style={{ fontSize: 17, fontStyle: 'italic', color: 'var(--sub)', lineHeight: 1.82, marginBottom: 16 }}>{watch.pitch}</div>
      <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: 12, color: 'var(--gold-lt)', letterSpacing: '.07em' }}>{watch.price}</div>
    </div>
  )
}
